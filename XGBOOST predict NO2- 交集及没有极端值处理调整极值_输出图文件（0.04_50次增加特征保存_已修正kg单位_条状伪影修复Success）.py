"""
海洋化学数据批量预测代码 - 修复经度条纹版 + 生产级功能
目标变量: G2nitrite
生成时间: 2025-08-08

结合功能:
1. 第二段代码的修复经度条纹版特征工程（优化热带条状伪影+修复纵向经度条纹）
2. 第一段代码的生产级批量预测功能（GPU加速+流式处理+GUI界面+内存管理+断点续传+单位换算）

修复特性:
- 移除经度带特征 (解决纵向"山脊"效应)
- 移除二倍频sin/cos变换 (减少密集纵向条纹)
- 使用pd.cut平滑分箱 (避免离散网格ID的硬边界)
- 保留优化纬度带特征 (针对-15°到15°条状伪影，11个纬度带)
...
def select_model_folder():
    ...
    try:
        messagebox.showinfo("第一步：选择模型文件夹",
                            "请选择包含以下文件的模型文件夹：\n"
                            "- G2nitrite_complete_model_longitude_stripes_fixed.json\n"
                            "- G2nitrite_complete_feature_engineering_info_longitude_stripes_fixed.pkl")
...
def get_model_folder_from_input():
    ...
    print("=== 模型文件夹路径设置 ===")
    print("需要包含以下文件：")
    print("- G2nitrite_complete_model_longitude_stripes_fixed.json")
    print("- G2nitrite_complete_feature_engineering_info_longitude_stripes_fixed.pkl")
...
def get_data_folders_from_input():
    ...
    print("特征工程：修复经度条纹版 (优化热带条状伪影+修复纵向经度条纹)")
...
def load_model_and_info_gpu(model_folder):
    ...
    MODEL_FILE = model_path / 'G2nitrite_complete_model_longitude_stripes_fixed.json'
    FEATURE_INFO_FILE = model_path / 'G2nitrite_complete_feature_engineering_info_longitude_stripes_fixed.pkl'

    print(f"📁 模型文件夹: {model_folder}")
    print(f"🔍 查找文件 (修复经度条纹版):")
    ...
    try:
        ...
        if not MODEL_FILE.exists():
            ...
            print(f"   请确保文件夹中包含: G2nitrite_complete_model_longitude_stripes_fixed.json")
            return None, None, False, None

        if not FEATURE_INFO_FILE.exists():
            print(f"❌ 特征工程信息文件不存在: {FEATURE_INFO_FILE}")
            print(f"   请确保文件夹中包含: G2nitrite_complete_feature_engineering_info_longitude_stripes_fixed.pkl")
            return None, None, False, None
...
def create_smooth_latitude_bands(latitude, feature_info):
    """创建平滑纬度带特征（优化版：针对-15°到15°条状伪影）"""
    spatial_info = feature_info.get('spatial_features', {})
    lat_band_info = spatial_info.get('smooth_latitude_bands', {})

    # 优化后的纬度带中心点 (11个纬度带)
    band_centers = lat_band_info.get('band_centers', [
        -60, -30, -23.5, -15, -7.5, 0, 7.5, 15, 23.5, 30, 60
    ])
    width = lat_band_info.get('width_parameter', 5.0)
    tropical_width_factor = lat_band_info.get('tropical_width_factor', 0.6)

    smooth_bands = {}

    for center in band_centers:
        # 对热带区域(-15°到15°)使用更小的宽度参数
        if -15 <= center <= 15:
            local_width = width * tropical_width_factor
        else:
            local_width = width

        # 使用sigmoid函数创建平滑过渡
        distance = np.abs(latitude - center)
        smooth_value = 1.0 / (1.0 + np.exp((distance - local_width/2) / (local_width/4)))

        # 创建特征名
        if center == 0:
            band_name = 'smooth_lat_band_equator'
        elif center > 0:
            if center == int(center):
                band_name = f'smooth_lat_band_{int(center)}N'
            else:
                band_name = f'smooth_lat_band_{center}N'.replace('.', 'p')
        else:
            if center == int(center):
                band_name = f'smooth_lat_band_{int(abs(center))}S'
            else:
                band_name = f'smooth_lat_band_{abs(center)}S'.replace('.', 'p')

        smooth_bands[band_name] = smooth_value

    return smooth_bands


def create_smooth_spatial_grid(latitude, longitude, feature_info):
    """创建平滑空间网格特征（修复版：使用pd.cut平滑分箱）"""
    spatial_info = feature_info.get('spatial_features', {})
    grid_info = spatial_info.get('smooth_spatial_grids', {})

    grid_sizes = grid_info.get('grid_sizes', [10, 5])

    grid_features = {}

    for grid_size in grid_sizes:
        # 使用pd.cut创建平滑的纬度和经度分箱
        lat_bins = np.linspace(-90, 90, int(180/grid_size) + 1)
        lon_bins = np.linspace(-180, 180, int(360/grid_size) + 1)

        # 使用pd.cut而不是np.floor，获得更平滑的分箱
        lat_grid = pd.cut(latitude, bins=lat_bins, labels=False, include_lowest=True)
        lon_grid = pd.cut(longitude, bins=lon_bins, labels=False, include_lowest=True)

        # 归一化到[0,1]范围，避免大的数值
        lat_grid_norm = lat_grid / (len(lat_bins) - 1)
        lon_grid_norm = lon_grid / (len(lon_bins) - 1)

        # 使用连续的相对位置而不是离散的网格ID
        grid_features[f'lat_grid_{grid_size}deg'] = lat_grid_norm
        grid_features[f'lon_grid_{grid_size}deg'] = lon_grid_norm

    return grid_features


def create_smooth_longitude_features(longitude, feature_info):
    """处理经度的平滑特征（修复版：移除经度带和二倍频）"""
    # 只保留基本的sin/cos变换，移除二倍频和经度带特征
    lon_rad = np.deg2rad(longitude)
    smooth_features = {
        'lon_sin': np.sin(lon_rad),
        'lon_cos': np.cos(lon_rad)
        # 已移除：lon_sin_2x, lon_cos_2x (二倍频)
        # 已移除：经度带特征 (lon_band_xxx)
    }
...
def apply_feature_engineering(df, feature_info):
    """
    应用特征工程（修复经度条纹版：优化热带条状伪影+修复纵向经度条纹）
    """
    print("🔧 应用修复经度条纹版特征工程（优化热带条状伪影+修复纵向经度条纹）...")
    ...
    for step in steps:
        if step == 'enhanced_geographic_time_features_longitude_stripes_fixed':
            print("  - 增强版地理时间特征（修复经度条纹版）")
            if 'G2latitude' in df_fe.columns and 'G2longitude' in df_fe.columns:
                ...
                # 修复版：平滑经度特征（移除经度带和二倍频）
                smooth_lon_features = create_smooth_longitude_features(df_fe['G2longitude'], feature_info)
                for name, values in smooth_lon_features.items():
                    df_fe[name] = values

                # 保留：优化平滑纬度带特征（针对热带条状伪影）
                smooth_lat_features = create_smooth_latitude_bands(df_fe['G2latitude'], feature_info)
                for name, values in smooth_lat_features.items():
                    df_fe[name] = values

                # 修复版：平滑空间网格特征（使用pd.cut避免硬边界）
                grid_features = create_smooth_spatial_grid(df_fe['G2latitude'], df_fe['G2longitude'], feature_info)
                for name, values in grid_features.items():
                    df_fe[name] = values
...
def batch_predict_nitrite_optimized(input_folder, output_folder, model, feature_info, use_gpu, gpu_id):
    """修复经度条纹版批量预测主函数"""
    print(f"🌊 开始批量 G2nitrite 预测（修复经度条纹版）...")
    ...
    print(f"特征工程: 修复经度条纹版 (优化热带条状伪影+修复纵向经度条纹)")
    ...
    for csv_file in progress_bar:
        ...
        try:
            ...
            # 生成输出文件名
            output_filename = f"predicted_{csv_file.stem}_longitude_stripes_fixed.csv"
    ...
    # 保存处理报告
    report_file = output_path / "batch_prediction_report_longitude_stripes_fixed.txt"
    with open(report_file, 'w', encoding='utf-8') as f:
        f.write(f"海洋化学数据批量预测报告 (修复经度条纹版 + 生产级功能)\n")
        ...
        f.write(f"特征工程版本: 修复经度条纹版\n")
        f.write(f"修复特性:\n")
        f.write(f"  ✅ 移除经度带特征 (解决纵向'山脊'效应)\n")
        f.write(f"  ✅ 移除二倍频sin/cos变换 (减少密集纵向条纹)\n")
        f.write(f"  ✅ 使用pd.cut平滑分箱 (避免离散网格ID的硬边界)\n")
        f.write(f"  ✅ 保留优化纬度带特征 (针对-15°到15°条状伪影，11个纬度带)\n\n")
        ...
        f.write(f"  ✅ GUI界面选择文件夹\n")
        f.write(f"  ✅ 内存管理和垃圾回收\n")
        f.write(f"  ✅ 断点续传功能\n")
        f.write(f"  ✅ 营养盐单位自动换算\n")
        f.write(f"  ✅ 完整的错误处理和报告\n")
        f.write(f"  ✅ 实时内存监控\n")
        f.write(f"  ✅ 自动垃圾回收\n\n")

        f.write(f"技术特性:\n")
        f.write(f"  ✅ 修复经度条纹版特征工程\n")
        f.write(f"  ✅ 11个优化纬度带特征\n")
        f.write(f"  ✅ 平滑空间网格处理\n")
...
def main():
    ...
    print("🔧 修复经度条纹版 + 生产级功能:")
    print("   - 修复经度条纹版特征工程 (优化热带条状伪影+修复纵向经度条纹)")
    ...
    print(f"\n🔧 特征工程版本: 修复经度条纹版")
    print(f"   修复特性:")
    print(f"   ✅ 移除经度带特征 (解决纵向'山脊'效应)")
    print(f"   ✅ 移除二倍频sin/cos变换 (减少密集纵向条纹)")
    print(f"   ✅ 使用pd.cut平滑分箱 (避免离散网格ID的硬边界)")
    print(f"   ✅ 保留优化纬度带特征 (针对-15°到15°条状伪影，11个纬度带)")
    ...
    # 开始批量预测
    print(f"\n{'=' * 80}")
    print(f"🚀 开始批量预测处理 (修复经度条纹版 + 生产级功能)")
    print(f"{'=' * 80}")
    ...
    print(f"\n💡 使用说明:")
    print(f"   1. 预测结果文件命名格式: predicted_[原文件名]_longitude_stripes_fixed.csv")
    print(f"   2. 输出列包含: 年月经纬度深度、预测nitrite、原始nitrate")
    print(f"   3. 预测值已应用物理约束 (nitrite ≥ 0)")
    print(f"   4. 详细处理报告: batch_prediction_report_longitude_stripes_fixed.txt")
    print(f"   5. 大文件自动使用流式处理，支持断点续传")

    print(f"\n🔧 特征工程说明 (修复经度条纹版):")
    print(f"   ✅ 优化热带条状伪影: 在-15°到15°区域增加精细纬度带")
    print(f"   ✅ 修复纵向经度条纹: 移除经度带特征和二倍频变换")
    print(f"   ✅ 平滑空间网格: 使用pd.cut避免离散网格ID的硬边界")
    print(f"   ✅ 11个优化纬度带: [-60, -30, -23.5, -15, -7.5, 0, 7.5, 15, 23.5, 30, 60]")
    print(f"   ✅ 热带区域优化: 赤道、±7.5°、±15°精细化处理")
