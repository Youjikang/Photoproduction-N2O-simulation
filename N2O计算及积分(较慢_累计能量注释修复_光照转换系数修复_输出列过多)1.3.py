...
class OptimizedN2OCalculator:
    ...
    def save_year_results_streaming(self, year: int, concentration_file_path: str, integration_file_path: str) -> \
        ...
        try:
            ...
            if integration_file_path and os.path.exists(integration_file_path):
                ...
                self.log_message(
                    f"    流式复制积分文件: {os.path.basename(integration_file_path)} → {integration_output_file.name}")
                ...
                with open(integration_output_file, 'w', newline='', encoding='utf-8') as outfile:
                    for chunk in pd.read_csv(integration_file_path, chunksize=chunk_size):
                        chunk.to_csv(outfile, mode='a', header=first_chunk, index=False)
                        first_chunk = False
                        total_integration_rows += len(chunk)
                        del chunk
                        gc.collect()

                self.log_message(f"    积分文件复制完成: {total_integration_rows:,} 行")
    ...
    def start_calculation(self):
        ...
        self.log_message("=== 开始N2O光产速率流式计算 (已修复所有问题) ===")
        ...
        try:
            ...
            self.log_message("=== 流式N2O计算全部完成! (所有问题已修复) ===")
