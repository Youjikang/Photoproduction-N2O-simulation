...
class TifResamplerApp:
        ...
        try:
            ...
            for i, tif_path in enumerate(tif_files):
                try:
                    ...
                    # 重命名列并重新保存结果
                    result.rename(columns={'effective_radius': 'cloud_water_path'}, inplace=True)
    ...
    def plot_results(self, df, output_png_path, tif_filename):
        ...
        # 使用重命名后的列
        if 'cloud_water_path' in df.columns:
            value_column = 'cloud_water_path'
        else:
        ...
        cbar.set_label('Cloud Water Path')
        ...
        # 设置图表属性
        title = f'Global Distribution of Cloud Water Path'
        if date_str:
            title += f' ({date_str})'
        plt.title(title)
