...
class ImprovedOceanDataVisualizer:
    def __init__(self):
        ...
        # 字体大小设置 - 统一管理所有字体大小 (已调整为用户要求的数值)
        self.font_sizes = {
            'colorbar_label': 30,  # 颜色条标签字体大小
            'colorbar_ticks': 28,  # 颜色条刻度数字字体大小 (从24改为28)
            'grid_labels': 28,     # 全球图网格线标签字体大小 (从24改为28)
            'lat_ylabel': 28,      # 纬度图Y轴标签字体大小 (从24改为28)
            'lat_xlabel': 26,      # 纬度图X轴标签字体大小 (从24改为28)
            'lat_ticks': 26,       # 纬度图刻度数字字体大小 (从20改为22)
        }
    ...
    def create_latitudinal_plot(self, data_subset, variable, year=None):
        ...
        # 设置x轴标签 - 从0开始，不显示负值，只显示3个标签，0显示为"0"
        ...
        # 手动格式化x轴标签，0显示为"0"，其他显示3位小数
        ax.set_xticklabels([f'{tick:.3f}' if abs(tick) > 1e-6 else '0' for tick in x_ticks])
