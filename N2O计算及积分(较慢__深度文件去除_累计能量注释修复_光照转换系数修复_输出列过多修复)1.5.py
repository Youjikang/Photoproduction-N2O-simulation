...
class OptimizedN2OCalculator:
    ...
    def calculate_light_attenuation_from_file(self, matched_file_path: str, year: int) -> Optional[str]:
        ...
        try:
            # 先加载N盐数据，创建经纬度->最大深度的映射
            self.log_message(f"    加载N盐数据以确定计算深度...")
            ...
            # 创建经纬度->最大深度的映射
            location_max_depths = {}
            for idx, n_salt_row in n_salt_df.iterrows():
                lat = round(n_salt_row['n_salt_latitude'], 6)
                lon = round(n_salt_row['n_salt_longitude'], 6)
                depth = n_salt_row['n_salt_depth']

                location_key = (lat, lon)
                if location_key not in location_max_depths:
                    location_max_depths[location_key] = depth
                else:
                    location_max_depths[location_key] = max(location_max_depths[location_key], depth)

            self.log_message(f"    N盐深度映射创建完成: {len(location_max_depths):,} 个位置")
            ...
            for chunk_idx, matched_chunk in enumerate(pd.read_csv(matched_file_path, chunksize=chunk_size)):
                ...
                for idx, row in matched_chunk.iterrows():
                    lat = round(row['kd_latitude'], 6)
                    lon = round(row['kd_longitude'], 6)
                    location_key = (lat, lon)

                    # 使用N盐数据中该经纬度的最大深度
                    if location_key not in location_max_depths:
                        self.log_message(f"      警告: 位置({lat}, {lon})在N盐数据中未找到，跳过", "WARNING")
                        continue

                    max_depth_from_n_salt = location_max_depths[location_key]

                    if pd.isna(max_depth_from_n_salt) or max_depth_from_n_salt <= 0:
                        continue

                    # 使用N盐数据的最大深度，不再限制于self.max_calculation_depth
                    max_depth_used = max_depth_from_n_salt
                    ...
                    for depth in depths:
                        depth_result = {
                            ...
                            'longitude': lon,
                            'depth': depth,
                            'max_depth_from_n_salt': max_depth_from_n_salt,
            ...
            # 清理深度映射
            del location_max_depths
            gc.collect()

            if total_results > 0:
                self.log_message(f"    光衰减计算完成: {total_results:,} 个深度点已保存到临时文件")
                self.log_message(f"    ✓ 使用N盐数据中各位置的最大深度进行计算")
    ...
    def start_calculation(self):
        ...
        self.log_message("✓ 优化特性: 基于N盐文件各经纬度实际最大深度计算，更科学合理")
        ...
        try:
            ...
            messagebox.showinfo("计算完成",
                                ...
                                f"• 深度计算优化：基于N盐文件各经纬度实际最大深度，更科学合理\n"
