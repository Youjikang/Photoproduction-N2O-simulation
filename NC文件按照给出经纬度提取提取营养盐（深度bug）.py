...
def process_nc_file_with_coords(input_file, coords_dir, output_dir, depth_interval=10,
    ...
    try:
        ...
        if 'depth' in dataset.dims:
            ...
            print(f"深度范围: {depths.min()} 至 {depths.max()} 米")
            print(f"深度值数量: {len(depths)}")
        else:
            print("警告: 数据集中没有深度维度")
        ...
        if 'time' in dataset.dims:
            for t_idx in range(len(times)):
                ...
                try:
                    ...
                    # 处理每个深度
                    if 'depth' in time_slice.dims:
                        depths = time_slice.depth.values
                        depth_chunks = [depths[i:i + chunk_size] for i in range(0, len(depths), chunk_size)]

                        for depth_chunk in depth_chunks:
                            print(f"处理深度块 {depth_chunk[0]:.1f}-{depth_chunk[-1]:.1f} 米...")

                            for depth in depth_chunk:
                                d_idx = np.where(depths == depth)[0][0]
                                ...
                                for var_name in vars_to_process:
                                    if var_name in time_slice and len(time_slice[var_name].dims) >= 3:  # 包含深度维度
                                        # 获取该深度的变量数据
                                        var_data = time_slice[var_name].isel(depth=d_idx).values
            ...
            if year not in year_coords:
                ...
                if 'depth' in dataset.dims:
                    ...
                    for d_idx, depth in enumerate(depths):
                        ...
                        # 对每个变量进行处理
                        for var_name in vars_to_process:
                            if var_name in dataset and len(dataset[var_name].dims) >= 3:
                                # 获取该深度的变量数据
                                var_data = dataset[var_name].isel(depth=d_idx).values
                        ...
                        # 显示进度
                        if d_idx % 10 == 0:
                            print(f"已处理 {d_idx + 1}/{len(depths)} 个深度层")
                else:
                    # 没有深度维度
                    # 创建数据字典，初始化坐标数据
                    data_rows = []
