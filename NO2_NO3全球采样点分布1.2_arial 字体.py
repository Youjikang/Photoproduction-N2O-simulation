...
class OceanNitrateNitriteVisualizer:
    ...
    def setup_gridlines(self, ax):
        """设置网格线 - 统一的网格线配置"""
        # 首先添加30度间隔的网格线（不显示标签）- 使用cartopy默认样式
        gl_grid = ax.gridlines(draw_labels=False, dms=False, x_inline=False, y_inline=False,
                               xlocs=np.arange(-180, 181, 30), ylocs=np.arange(-90, 91, 30))

        # 然后添加60度间隔的标签（不显示额外网格线）
        gl_labels = ax.gridlines(draw_labels=True, dms=False, x_inline=False, y_inline=False,
                                 ...
                                 alpha=0.0, linewidth=0)  # 透明网格线，只显示标签

        gl_labels.top_labels = True
        gl_labels.right_labels = True

        # 设置格式为简单的正负数字（不使用E/W）
        gl_labels.xformatter = mticker.FuncFormatter(lambda x, pos: f'{int(x)}°')
        gl_labels.yformatter = mticker.FuncFormatter(lambda y, pos: f'{int(y)}°')

        gl_labels.xlabel_style = {'family': 'Arial', 'weight': 'bold'}
        gl_labels.ylabel_style = {'family': 'Arial', 'weight': 'bold'}
    ...
    def create_combined_original_grid(self, data_subset, variables, year_range=None):
        ...
        for variable in variables:
            ...
            # 确定颜色
            if color_config['color'] == 'lightseagreen':
                rgba_color = (0.13, 0.70, 0.67, 0.1)  # nitrate使用淡海绿色
            elif color_config['color'] == 'plum':
                rgba_color = (0.87, 0.63, 0.87, 0.1)  # nitrite使用淡plasma色（淡紫色）
            else:
                rgba_color = (0.0, 0.0, 1.0, 0.1)  # 默认蓝色
    ...
    def run(self):
        ...
        print("- 优化内存管理，支持大数据集")
        print("- 字体设置：Arial 20号粗体")
        print("- 网格线：经度30°间隔，标注60°间隔，使用正负数字（不使用E/W）")
        print()
        ...
        print("- 字体: Arial 20号粗体")
        print("- 网格设置: 经度30°间隔线，60°间隔标注，使用±数字格式")
        ...
        print(f"字体设置: Arial 20号粗体")
        print(f"网格设置: 经度网格30°间隔，标注60°间隔，正负数字格式")
        ...
        print(f"配色方案: nitrate=淡海绿色RGBA(0.13,0.70,0.67,0.1), nitrite=淡紫色RGBA(0.87,0.63,0.87,0.1)")
        print(f"海洋背景: 空白（白色）")
        ...
        print(f"字体设置: Arial 20号粗体")
        print(f"网格设置: 经度网格30°间隔，标注60°间隔，正负数字格式（不使用E/W）")
        print(f"{'=' * 60}")
