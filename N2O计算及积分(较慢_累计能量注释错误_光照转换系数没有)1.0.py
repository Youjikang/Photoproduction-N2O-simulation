...
class OptimizedN2OCalculator:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("N2O光产速率计算程序 - 优化版 (已修复西半球网格匹配)")
    ...
    def setup_gui(self):
        """设置图形用户界面"""
        # 主标题
        title_label = tk.Label(self.root, text="N2O光产速率计算程序 - 流式处理优化版 (已修复西半球网格匹配)",
        ...
        for band_name, params in self.n2o_params.items():
            ...
            if pd.isna(light_intensity_mw_per_m2) or light_intensity_mw_per_m2 <= 0:
                ...
                try:
                    ...
                    n2o_value = params['factor1'] * (exp_term - 1) * params['light_factor'] * annual_light_intensity_J
    ...
    def process_single_year_streaming(self, year: int) -> Tuple[Optional[pd.DataFrame], Optional[pd.DataFrame]]:
        ...
        try:
            ...
            if n2o_data_info['total_rows'] > 0:
                ...
                # 将临时文件转换为DataFrame返回（仅用于保存，不占用太多内存）
                concentration_df = pd.read_csv(n2o_file_path)

                # 删除N2O临时文件
                try:
                    os.remove(n2o_file_path)
                    self.log_message(f"    已删除临时文件: {os.path.basename(n2o_file_path)}")
                except:
                    pass

                return concentration_df, integration_df
            else:
    ...
    def save_year_results(self, year: int, concentration_df: pd.DataFrame, integration_df: pd.DataFrame) -> bool:
        """保存年度结果到输出文件夹"""
        try:
            # 创建年度输出子文件夹
            ...
            # 保存浓度数据
            concentration_file = year_output_dir / f"n2o_concentration_{year}.csv"
            concentration_df.to_csv(concentration_file, index=False)

            # 保存积分数据
            if integration_df is not None and len(integration_df) > 0:
                integration_file = year_output_dir / f"n2o_integrated_{year}.csv"
    ...
    def start_calculation(self):
        ...
        self.log_message("=== 开始N2O光产速率流式计算 (已修复西半球网格匹配) ===")
        ...
        try:
            ...
            for i, year in enumerate(available_years):
                ...
                # 流式处理单年数据
                concentration_df, integration_df = self.process_single_year_streaming(year)

                if concentration_df is not None and len(concentration_df) > 0:
                    # 保存年度结果
                    if self.save_year_results(year, concentration_df, integration_df):
                        year_output_dir = Path(self.output_folder) / f"year_{year}"
                        all_concentration_files.append(year_output_dir / f"n2o_concentration_{year}.csv")
                        if integration_df is not None and len(integration_df) > 0:
                            all_integration_files.append(year_output_dir / f"n2o_integrated_{year}.csv")

                    # 统计信息
                    valid_n2o = concentration_df[concentration_df['N2O_total'] > 0]
                    self.log_message(
                        f"  {year} 年统计: 总数据{len(concentration_df):,}, 有效N2O{len(valid_n2o):,}, 积分位置{len(integration_df) if integration_df is not None else 0:,}")

                    # 立即释放内存
                    del concentration_df
            ...
            self.log_message("=== 流式N2O计算全部完成! (网格匹配已修复) ===")
