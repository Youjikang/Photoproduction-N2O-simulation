            ...
            for name, model_info in models.items():
                ...
                try:
                    ...
                    if name == 'Fixed_Linear_Exp':
                        print(
                            f"N2O = [{popt[0]:.4f}×C_eq + {popt[1]:.4f}×(e^(0.1×C_eq)-1)] × ({popt[2]:.4f}×Light + {popt[3]:.4f}) + {popt[4]:.4f}")
                        print(f"             其中 C_eq = NO2 + {conversion_factor:.6f}×NO3")
                    elif name == 'Fixed_Power':
                        print(
                            f"N2O = {popt[0]:.4f} × C_eq^{popt[1]:.4f} × ({popt[2]:.4f}×Light + {popt[3]:.4f}) + {popt[4]:.4f}×Light + {popt[5]:.4f}")
                        print(f"             其中 C_eq = NO2 + {conversion_factor:.6f}×NO3")
                    elif name == 'Fixed_Piecewise':
                        print(f"N2O = 分段模型:")
                        print(f"             低浓度: {popt[0]:.4f} × C_eq × ({popt[2]:.4f}×Light + {popt[3]:.4f})")
                        print(
                            f"             高浓度: {popt[1]:.4f} × e^({popt[4]:.4f}×C_eq) × ({popt[2]:.4f}×Light + {popt[3]:.4f})")
                        print(f"             过渡函数: T = 1/(1 + e^(-10×(C_eq - 3.0)))")
                        print(f"             最终: (1-T)×低浓度 + T×高浓度 + {popt[5]:.4f}×Light + {popt[6]:.4f}")
                        print(f"             其中 C_eq = NO2 + {conversion_factor:.6f}×NO3")
                    elif name == 'Fixed_Exponential':
                        print(
                            f"N2O = {popt[0]:.4f} × e^({popt[1]:.4f}×C_eq) × ({popt[2]:.4f}×Light + {popt[3]:.4f}) + {popt[4]:.4f}×Light + {popt[5]:.4f}")
                        print(f"             其中 C_eq = NO2 + {conversion_factor:.6f}×NO3")
