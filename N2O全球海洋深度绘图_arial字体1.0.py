...
# 设置全局字体 - 字体大小翻倍，字体改为Arial
plt.rcParams['font.family'] = 'Arial'
plt.rcParams['font.size'] = 24  # 从12改为24
...
class ImprovedOceanDataVisualizer:
    ...
    def create_latitudinal_plot(self, data_subset, variable, depth, year=None):
        ...
        # 设置坐标轴 - 确保与全球图完全一致，字体大小翻倍，字体改为Arial
        ax.set_ylabel('Latitude', fontsize=24, fontweight='bold', family='Arial')
        # x轴标题字体大小缩小30%，从24改为16.8
        ax.set_xlabel(color_config['label'], fontsize=16.8, fontweight='bold', family='Arial')
    ...
    def create_scatter_plot(self, data_subset, variable, depth, year=None):
        ...
        # 再添加标签（经度60度间隔，纬度30度间隔）- 字体改为Arial
        gl = ax.gridlines(draw_labels=True, dms=True, x_inline=False, y_inline=False,
                          xlocs=np.arange(-180, 181, 60), ylocs=np.arange(-90, 91, 30))
        gl.top_labels = True
        gl.right_labels = True
        gl.xlabel_style = {'family': 'Arial', 'weight': 'bold', 'size': 20}
        gl.ylabel_style = {'family': 'Arial', 'weight': 'bold', 'size': 20}
    ...
    def run(self):
        """运行主程序"""
        print("=== 改进的全球海洋N2O数据可视化工具（Arial字体版本） ===")
        print("功能特性:")
        ...
        print("- 所有字体大小翻倍，使用Arial字体符合科研用途")
        print()
