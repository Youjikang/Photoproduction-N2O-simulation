...
class ImprovedOceanDataVisualizer:
    def __init__(self):
        ...
        # 为不同变量定义不同的配色方案 - 修改为kd相关变量
        self.color_schemes = {
            '320': {
                'cmap': self.selected_colormap,
                'label': r'$\mathit{K}$$_\mathrm{d}$(320) (m$^{-1}$)'
            },
            'kd_380': {
                'cmap': self.selected_colormap,
                'label': r'$\mathit{K}$$_\mathrm{d}$(380) (m$^{-1}$)'
            },
            'kd_443': {
                'cmap': self.selected_colormap,
                'label': r'$\mathit{K}$$_\mathrm{d}$(443) (m$^{-1}$)'
            },
            'kd_490': {
                'cmap': self.selected_colormap,
                'label': r'$\mathit{K}$$_\mathrm{d}$(490) (m$^{-1}$)'
            },
            'kd_555': {
                'cmap': self.selected_colormap,
                'label': r'$\mathit{K}$$_\mathrm{d}$(555) (m$^{-1}$)'
            }
        }

        # 图形尺寸设置 - 确保纵向比例一致
        self.global_fig_size = (16, 10)  # 全球图尺寸
        self.lat_fig_size = (4, 8)  # 纬度图尺寸，纵向高度与全球图匹配
    ...
    def get_color_config(self, variable):
        ...
        if variable in self.color_schemes:
            ...
            # 默认配色 - 修改为kd通用标签
            if variable.isdigit():
                label = rf'$\mathit{{K}}$$_\mathrm{{d}}$({variable}) (m$^{{-1}}$)'
            else:
                label = rf'$\mathit{{K}}$$_\mathrm{{d}}$ ({variable}) (m$^{{-1}}$)'
            return {
                'cmap': self.selected_colormap,
    ...
    def set_manual_color_limits(self):
        """设置手动颜色范围 - 修改为kd变量"""
        variables = ['320', 'kd_380', 'kd_443', 'kd_490', 'kd_555']  # 修改变量列表
    ...
    def load_single_csv(self, file_path):
        """加载单个CSV文件，优化内存使用 - 修改为kd相关列"""
        try:
            print(f"  加载: {os.path.basename(file_path)}")

            # 只读取需要的列，减少内存使用 - 修改为kd相关列
            required_columns = ['longitude', 'latitude', '320']  # kd320列名为'320'
            optional_columns = ['year', 'kd_380', 'kd_443', 'kd_490', 'kd_555']  # 其他可能的kd列
    ...
    def select_variables(self):
        """选择要绘制的变量（可多选）- 修改为kd变量"""
        variables = ['320', 'kd_380', 'kd_443', 'kd_490', 'kd_555']  # 修改为kd相关变量
        ...
        variable_descriptions = {
            '320': 'Kd320 - 320nm波长光衰减系数',
            'kd_380': 'Kd380 - 380nm波长光衰减系数',
            'kd_443': 'Kd443 - 443nm波长光衰减系数',
            'kd_490': 'Kd490 - 490nm波长光衰减系数',
            'kd_555': 'Kd555 - 555nm波长光衰减系数'
        }

        for i, var in enumerate(variables):
            desc = variable_descriptions.get(var, f'{var} - 光衰减系数')
            print(f"{i + 1}. {desc} (配色: {colormap_name})")
        print("=" * 60)
        print("\n输入示例:")
        print("- 选择单个变量: 输入 1 (选择Kd320)")
        print("- 选择多个变量: 输入 1,2,3")
        print("- 选择所有变量: 输入 1,2,3,4,5")
        ...
        while True:
            try:
                ...
                if not var_input:
                    print("未输入任何内容，使用默认变量: 320 (Kd320)")
                    return ['320']
    ...
    def format_coordinate_labels(self, ax, is_longitude=True):
        """格式化坐标标签，使用正负号而不是NWSE"""
        if is_longitude:
            # 经度标签：60度间隔
    ...
    def create_scatter_plot(self, data_subset, variable, year=None):
        ...
        # 单独添加标签 - 经度标签60度间隔，纬度标签30度间隔
        gl_labels = ax.gridlines(draw_labels=True, dms=False, x_inline=False, y_inline=False,
        ...
        gl_labels.xformatter = plt.FuncFormatter(format_lon)
        gl_labels.yformatter = plt.FuncFormatter(format_lat)

        gl_labels.xlabel_style = {'family': 'Arial', 'weight': 'bold', 'size': 28}
        gl_labels.ylabel_style = {'family': 'Arial', 'weight': 'bold', 'size': 28}
    ...
    def create_high_res_grid(self, data_subset, variable, year=None):
        ...
        # 添加网格线和标签 - 修正：经度网格30度间隔，经度标签60度间隔
    ...
    def process_single_file(self, csv_file, variables):
        ...
        try:
            ...
            for variable in variables:
                ...
                year_data = year_data[year_data[variable] <= 100]
                filtered_count = len(year_data)
                removed_count = original_count - filtered_count

                if removed_count > 0:
                    print(f"      数据预处理: 移除了 {removed_count} 个大于100的异常值")
                    print(f"      剩余数据点: {filtered_count} (原始: {original_count})")
                else:
                    print(f"      数据预处理: 无异常值，数据点数量: {filtered_count}")
                # ===== 数据预处理结束 =====

                if len(year_data) == 0:
                    print(f"      ✗ 变量 {variable} 经过预处理后没有有效数据")
                    continue

                print(f"      最终有效数据点: {len(year_data)}")
    ...
    def run(self):
        """运行主程序"""
        print("=== 改进的全球海洋Kd数据可视化工具（含平均值分析与数据预处理） ===")
        print("🔥 新增功能:")
        ...
        print("- 优化内存管理，避免内存崩溃")
        print("- 大数据集自动采样")
        print("- 支持Kd(光衰减系数)相关参数可视化")
        print("- 特别支持Kd320列名为'320'的数据格式")
        print("- ⭐ 数据预处理：自动去除大于100的异常值")
        print("- ⭐ 网格优化：经度网格30度间隔，标签60度间隔")
        print()
        ...
        print("⚠️  注意：程序将自动去除所有大于100的异常值")
        print("⚠️  网格设置：经度网格30度间隔，经度标签60度间隔")
        ...
        print("✅ 数据预处理: 已自动去除所有大于100的异常值")
        print("✅ 网格设置: 经度网格30度间隔，经度标签60度间隔")
