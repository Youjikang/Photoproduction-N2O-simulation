...
class OptimizedN2OCalculator:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("N2O光产速率计算程序 - 优化版 (已修复西半球网格匹配+光能缩放因子+大文件内存)")
        ...
        # 单位转换常数
        self.seconds_per_day = 86400  # 1天 = 86400秒
        self.mw_to_w = 1 / 1000  # mW转换为W：除以1000
    ...
    def setup_gui(self):
        ...
        title_label = tk.Label(self.root,
                               text="N2O光产速率计算程序 - 流式处理优化版 (已修复西半球网格匹配+光能缩放因子+大文件内存)",
        ...
        fix_label = tk.Label(self.root,
                             text="✓ 已修复西半球负经度网格匹配问题\n"
                                  "✓ 西半球数据匹配精度提升10度\n"
                                  "✓ 已添加波段光能缩放因子 (UVB÷1000, UVA÷1000000, BLUE÷1000000)\n"
        ...
        info_label = tk.Label(self.root,
                              text="计算海洋中N2O光产速率 (1997-2022年数据)\n"
                                   ...
                                   "输出: N2O浓度(nmol-N/L) 和 深度积分(nmol-N/m²)",
                              font=("Arial", 10))
    ...
    def test_grid_matching(self):
        ...
        self.log_message("✓ 光能缩放因子已应用: UVB÷1000, UVA÷1000000, BLUE÷1000000")
    ...
    def calculate_n2o_production(self, nitrite: float, nitrate: float, light_intensities: dict, year: int) -> dict:
        ...
        for band_name, params in self.n2o_params.items():
            light_intensity_mw_per_m2 = light_intensities.get(f'{band_name}_intensity', 0)

            if pd.isna(light_intensity_mw_per_m2) or light_intensity_mw_per_m2 <= 0:
                band_n2o[band_name] = 0.0
                results[f'N2O_{band_name}'] = 0.0
                results[f'N2O_{band_name}_annual_light_J'] = 0.0
                results[f'N2O_{band_name}_scaled_light_J'] = 0.0
            else:
                # 单位转换
                conversion_factor = self.mw_to_w * self.seconds_per_day
                annual_light_intensity_J = light_intensity_mw_per_m2 * days_in_year * conversion_factor

                # 应用波段缩放因子
                scaling_factor = band_scaling_factors.get(band_name, 1.0)
                scaled_annual_light_J = annual_light_intensity_J * scaling_factor
                ...
                try:
                    ...
                    n2o_value = params['factor1'] * (exp_term - 1) * params['light_factor'] * scaled_annual_light_J
                    n2o_value = max(0.0, n2o_value)
                    band_n2o[band_name] = n2o_value
                    results[f'N2O_{band_name}'] = n2o_value
                    results[f'N2O_{band_name}_annual_light_J'] = annual_light_intensity_J
                    results[f'N2O_{band_name}_scaled_light_J'] = scaled_annual_light_J
    ...
    def process_single_year_streaming(self, year: int) -> Tuple[Optional[str], Optional[pd.DataFrame]]:
        """流式处理单年数据 - 使用临时文件减少内存压力"""
        ...
        try:
            ...
            # 4. 从临时文件计算深度积分
            self.log_message(f"  步骤4: 深度积分计算")
            integration_df = self.calculate_depth_integration_from_file(n2o_file_path, year)
            ...
            if n2o_data_info['total_rows'] > 0:
                ...
                self.log_message(f"    文件大小: {n2o_data_info['file_size_mb']:.1f} MB")
                self.log_message(f"    积分数据: {len(integration_df) if integration_df is not None else 0:,} 条")
                self.log_message(f"    ✓ 使用流式处理，未加载到内存")

                # 返回临时文件路径而不是DataFrame，避免大文件加载到内存
                return n2o_file_path, integration_df
            else:
                ...
                for idx, row in attenuation_chunk.iterrows():
                    ...
                    if n_salt_row is not None:
                        ...
                        for band_name in self.bands.keys():
                            ...
                            result_row[f'N2O_{band_name}_annual_light_J'] = np.nan
    ...
    def calculate_depth_integration_from_file(self, n2o_file_path: str, year: int) -> Optional[pd.DataFrame]:
        """从临时文件读取N2O数据并计算深度积分"""
        self.log_message(f"    从临时文件计算 {year} 年N2O深度积分...")

        try:
            # 分块读取N2O数据并按位置分组
            location_data = {}  # {(lat, lon): [list of rows]}
            chunk_size = 10000

            for chunk_idx, n2o_chunk in enumerate(pd.read_csv(n2o_file_path, chunksize=chunk_size)):
                self.log_message(f"      读取深度积分块 {chunk_idx + 1}, 行数: {len(n2o_chunk):,}")
                ...
                for idx, row in n2o_chunk.iterrows():
                    if pd.notna(row['N2O_total']) and row['N2O_total'] > 0:
                        ...
                        location_data[location_key].append(row)
            ...
            if not location_data:
                self.log_message(f"    无有效N2O数据进行积分", "WARNING")
                return pd.DataFrame()

            self.log_message(f"    共收集到 {len(location_data):,} 个位置的N2O数据")

            # 对每个位置进行深度积分
            integration_results = []
            processed_locations = 0

            for location_key, location_rows in location_data.items():
                ...
                integration_result = {
                    ...
                    'latitude': lat,
                    'longitude': lon,
                    'integration_depth_min': depths.min(),
                    'integration_depth_max': depths.max(),
                    'integration_depth_range': depths.max() - depths.min(),
                    'integration_points': len(valid_n2o_data),
                    'N2O_integrated_nmol_N_per_m2': integrated_n2o_per_m2,
                    'N2O_mean_concentration_nmol_N_per_L': n2o_concentrations.mean(),
                    'N2O_max_concentration_nmol_N_per_L': n2o_concentrations.max(),
                    'N2O_min_concentration_nmol_N_per_L': n2o_concentrations.min()
                }

                # 添加各波段的深度积分
                for band_name in self.bands.keys():
                    band_concentrations = valid_n2o_data[f'N2O_{band_name}'].values
                ...
                # 添加基础信息
                first_valid_point = valid_n2o_data.iloc[0]
                for key in ['days_in_year', 'max_depth_available', 'max_depth_used']:
                    if key in first_valid_point:
                        integration_result[key] = first_valid_point[key]

                integration_results.append(integration_result)
                processed_locations += 1

                # 进度更新
                if processed_locations % 1000 == 0:
                    self.log_message(f"      积分进度: {processed_locations:,}/{len(location_data):,}")
            ...
            if integration_results:
                ...
                self.log_message(f"    深度积分完成: {len(integration_df):,} 个位置")
                return integration_df
            else:
                self.log_message(f"    深度积分失败: 无有效数据", "ERROR")
                return pd.DataFrame()

        except Exception as e:
            self.log_message(f"    深度积分失败: {str(e)}", "ERROR")
            return pd.DataFrame()

    def save_year_results_streaming(self, year: int, concentration_file_path: str, integration_df: pd.DataFrame) -> \
    Tuple[bool, Path, Path]:
        """流式保存年度结果到输出文件夹（避免加载大文件到内存）"""
        try:
            # 创建年度输出子文件夹
            ...
            self.log_message(
                f"    流式复制浓度文件: {os.path.basename(concentration_file_path)} → {concentration_output_file.name}")
            ...
            with open(concentration_output_file, 'w', newline='', encoding='utf-8') as outfile:
                for chunk in pd.read_csv(concentration_file_path, chunksize=chunk_size):
                    # 第一块包含表头，后续块不包含表头
                    chunk.to_csv(outfile, mode='a', header=first_chunk, index=False)
                    first_chunk = False
                    total_copied_rows += len(chunk)

                    # 清理内存
                    del chunk
                    gc.collect()

            self.log_message(f"    浓度文件复制完成: {total_copied_rows:,} 行")

            # 保存积分数据（通常很小，可以直接保存）
            integration_output_file = None
            if integration_df is not None and len(integration_df) > 0:
                integration_output_file = year_output_dir / f"n2o_integrated_{year}.csv"
                integration_df.to_csv(integration_output_file, index=False)
                self.log_message(f"    积分文件保存完成: {len(integration_df):,} 行")
            ...
            self.log_message(f"  {year} 年结果已保存到: {year_output_dir}")
    ...
    def start_calculation(self):
        ...
        self.log_message("=== 开始N2O光产速率流式计算 (已修复西半球网格匹配+光能缩放因子+大文件内存) ===")
        ...
        try:
            ...
            for i, year in enumerate(available_years):
                ...
                # 流式处理单年数据
                concentration_file_path, integration_df = self.process_single_year_streaming(year)
                ...
                if concentration_file_path is not None:
                    ...
                    save_success, saved_concentration_file, saved_integration_file = self.save_year_results_streaming(
                        year, concentration_file_path, integration_df)
                    ...
                    if save_success:
                        ...
                        self.log_message(
                            f"  {year} 年统计: 总数据{file_info['total_rows']:,}, 有效N2O{file_info['valid_n2o_rows']:,}, 积分位置{len(integration_df) if integration_df is not None else 0:,}")
            ...
            if all_concentration_files:
                ...
                self.merge_csv_files_streaming(all_concentration_files, combined_concentration_file)
            ...
            if all_integration_files:
                ...
                self.merge_csv_files_streaming(all_integration_files, combined_integration_file)
            ...
            self.log_message("=== 流式N2O计算全部完成! (网格匹配+光能缩放+大文件内存已修复) ===")
    ...
    def merge_csv_files_streaming(self, file_list: List[Path], output_file: Path):
        """流式合并CSV文件（避免同时加载所有文件到内存）"""
        if not file_list:
            return
        ...
        try:
            ...
            with open(output_file, 'w', newline='', encoding='utf-8') as outfile:
                ...
                for i, file_path in enumerate(file_list):
                    ...
                    for chunk in pd.read_csv(file_path, chunksize=chunk_size):
                        # 写入数据（不包含表头）
                        chunk.to_csv(outfile, mode='a', header=False, index=False)

                    # 强制垃圾回收
                    gc.collect()

            self.log_message(f"  合并完成: {output_file}")

        except Exception as e:
            self.log_message(f"  合并失败: {str(e)}", "ERROR")
