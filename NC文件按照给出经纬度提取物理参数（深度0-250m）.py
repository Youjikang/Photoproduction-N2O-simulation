...
def process_nc_file_with_coords(input_file, coords_dir, output_dir, depth_interval=10,
    ...
    try:
        ...
        if 'depth' in dataset.dims:
            ...
            # 修改: 使用固定的深度范围 - 从10m到250m，以10m为间隔
            # 同时保存最小深度的原始值用于后续赋值给0m深度
            min_orig_depth_idx = np.argmin(orig_depths)
            print(f"原始数据最小深度: {orig_depths[min_orig_depth_idx]} 米")

            # 创建新的深度层序列: 0m, 10m, 20m, ..., 250m
            new_depths = np.concatenate(([0], np.arange(10, 260, depth_interval)))
            print(f"将创建 {len(new_depths)} 个新深度层")
            print(f"新深度范围: {new_depths.min()} 至 {new_depths.max()} 米")
            print(f"深度层: {new_depths}")
        else:
            print("警告: 数据集中没有深度维度")
        ...
        if 'time' in dataset.dims:
            for t_idx in range(len(times)):
                ...
                try:
                    ...
                    # 处理深度维度
                    if has_depth:
                        # 修改: 先用原始深度进行插值到10m-250m的深度
                        print("使用xarray的interp方法进行深度插值...")

                        # 先对深度>0的部分进行插值 (10m到250m)
                        interp_depths = new_depths[new_depths > 0]
                        interp_dataset = time_slice.interp(depth=interp_depths, method='linear')

                        # 单独处理0米深度 - 使用原始数据中最小深度的值
                        zero_depth_dataset = time_slice.isel(depth=min_orig_depth_idx)
                        ...
                        for depth_idx, new_depth in enumerate(new_depths):
                            ...
                            if new_depth == 0:
                                # 对0米深度，使用最小深度的数据
                                depth_slice = zero_depth_dataset
                            else:
                                # 对其他深度，使用插值数据
            ...
            if year not in year_coords:
                ...
                if has_depth:
                    ...
                    # 先对深度>0的部分进行插值 (10m到250m)
                    interp_depths = new_depths[new_depths > 0]
                    interp_dataset = dataset.interp(depth=interp_depths, method='linear')

                    # 单独处理0米深度 - 使用原始数据中最小深度的值
                    zero_depth_dataset = dataset.isel(depth=min_orig_depth_idx)
...
def main():
    ...
    # 深度间隔 - 固定为10米
    depth_interval = 10
    print(f"深度间隔已固定为: {depth_interval}米")
    print("将从0米到250米按10米间隔生成深度层")
    ...
    print(f"深度间隔: {depth_interval}米 (将用于生成0米至250米的深度层)")
