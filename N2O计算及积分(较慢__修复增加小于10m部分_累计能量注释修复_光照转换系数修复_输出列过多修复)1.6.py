...
class OptimizedN2OCalculator:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title(
            "N2O光产速率计算程序 - 优化版 (已修复西半球网格匹配+光能单位转换+流式深度积分+大文件内存+深度分辨率自适应)")
        ...
        # 计算参数 - 修改为自适应深度分辨率
        self.standard_depth_resolution = 10  # 标准深度分辨率：10米
        self.shallow_depth_resolution = 1  # 浅水深度分辨率：1米
        self.shallow_depth_threshold = 10  # 浅水深度阈值：10米
        ...
        # 定义输出文件需要的列 - 修改增加N盐匹配信息和波段积分光强
        self.concentration_output_columns = [
            'latitude', 'longitude', 'depth',
            'N2O_UVB', 'N2O_UVA', 'N2O_BLUE', 'N2O_total',
            'matched_nitrite', 'matched_nitrate',
            'n_salt_match_lat_diff', 'n_salt_match_lon_diff', 'n_salt_match_depth_diff',
            'UVB_intensity', 'UVA_intensity', 'BLUE_intensity'
        ]
    ...
    def setup_gui(self):
        ...
        fix_label = tk.Label(self.root,
                             text="✓ 已修复西半球负经度网格匹配问题\n"
                                  ...
                                  "✓ 增加N盐匹配信息输出：匹配差异、浓度值等详细信息\n"
                                  "✓ 新增深度自适应分辨率：深度<10m使用1m分辨率，>=10m使用10m分辨率\n"
                                  "✓ 新增浅水特殊处理：深度<10m使用0m的N盐数据作为整个水柱数据\n"
                                  "✓ 建议重新处理1997-2022年数据",
        ...
        info_label = tk.Label(self.root,
                              text="计算海洋中N2O光产速率 (1997-2022年数据)\n"
                                   "优化特性: 流式处理、低内存占用、SSD优化、自适应深度分辨率\n"
                                   "输出: N2O浓度(nmol-N/L) 和 深度积分(nmol-N/m²) - 包含详细匹配信息\n"
                                   "深度处理: <10m使用1m分辨率，>=10m使用10m分辨率",
                              font=("Arial", 10))
        ...
        # 测试深度分辨率按钮 - 新增
        test_depth_button = tk.Button(button_frame, text="测试深度分辨率",
                                      command=self.test_depth_resolution, bg="lightpink", width=12)
        test_depth_button.pack(side="left", padx=8)
    ...
    def test_depth_resolution(self):
        """测试深度分辨率处理功能"""
        self.log_message("=== 测试深度自适应分辨率处理 ===")

        # 测试案例
        test_cases = [
            {"max_depth": 5.0, "description": "极浅水域 5m"},
            {"max_depth": 8.5, "description": "浅水域 8.5m"},
            {"max_depth": 10.0, "description": "临界深度 10m"},
            {"max_depth": 15.0, "description": "中等深度 15m"},
            {"max_depth": 25.0, "description": "标准深度 25m"},
            {"max_depth": 300.0, "description": "超深水域 300m"}
        ]

        self.log_message("测试各种深度的分辨率处理:")
        for case in test_cases:
            max_depth = case["max_depth"]
            desc = case["description"]

            depths, resolution_used, max_depth_used = self.calculate_depth_points(max_depth)

            self.log_message(f"  {desc}:")
            self.log_message(f"    原始深度: {max_depth} m")
            self.log_message(f"    实际使用深度: {max_depth_used} m")
            self.log_message(f"    分辨率: {resolution_used} m")
            self.log_message(f"    深度点数: {len(depths)} 个")
            self.log_message(f"    深度序列: {depths[:10]}{' ...' if len(depths) > 10 else ''}")

            # 判断是否使用浅水处理
            if max_depth < self.shallow_depth_threshold:
                self.log_message(f"    ✓ 使用浅水处理：1m分辨率 + 0m的N盐数据")
            else:
                self.log_message(f"    ✓ 使用标准处理：10m分辨率 + 深度匹配N盐数据")
            self.log_message("")

        self.log_message("深度分辨率测试说明:")
        self.log_message(f"• 浅水阈值: {self.shallow_depth_threshold} m")
        self.log_message(f"• 浅水分辨率: {self.shallow_depth_resolution} m")
        self.log_message(f"• 标准分辨率: {self.standard_depth_resolution} m")
        self.log_message(f"• 最大计算深度: {self.max_calculation_depth} m")
        self.log_message("• 浅水区域(<10m)使用0m的N盐数据，避免数据缺失")
        self.log_message("• 标准区域(>=10m)使用深度匹配的N盐数据")
        self.log_message("=== 深度分辨率测试完成 ===")

    def calculate_depth_points(self, max_depth_available: float) -> Tuple[np.ndarray, int, float]:
        """计算深度点序列 - 自适应分辨率"""
        # 确定实际使用的最大深度
        max_depth_used = min(max_depth_available, self.max_calculation_depth)

        # 根据深度选择分辨率
        if max_depth_used < self.shallow_depth_threshold:
            # 浅水区域：使用1m分辨率
            resolution = self.shallow_depth_resolution
            depths = np.arange(0, max_depth_used + resolution, resolution)
        else:
            # 标准区域：使用10m分辨率
            resolution = self.standard_depth_resolution
            depths = np.arange(0, max_depth_used + resolution, resolution)

        # 确保不超过最大深度
        depths = depths[depths <= max_depth_used]

        return depths, resolution, max_depth_used
    ...
    def find_matching_n_salt_data_adaptive(self, lat: float, lon: float, depth: float,
                                           n_salt_df: pd.DataFrame, spatial_index: Dict,
                                           is_shallow: bool = False) -> Optional[pd.Series]:
        """自适应N盐数据查找 - 支持浅水区域特殊处理"""

        if is_shallow:
            # 浅水区域：强制使用0m深度的N盐数据
            target_depth = 0.0
            self.log_message(f"      浅水处理: 使用0m深度的N盐数据 (原深度: {depth}m)", "DEBUG")
        else:
            # 标准区域：使用实际深度
            target_depth = depth
        ...
        lat_key = round(lat, 3)
        lon_key = round(lon, 3)
        depth_key = round(target_depth, 0)
        ...
        for idx in candidate_indices:
            ...
            depth_diff = abs(row['n_salt_depth'] - target_depth)  # 使用目标深度
    ...
    def process_single_year_streaming(self, year: int) -> Tuple[Optional[str], Optional[str]]:
        """流式处理单年数据 - 修复版本"""
        self.log_message(f"=== 开始处理 {year} 年数据 (临时文件模式 + 自适应深度分辨率) ===")
        ...
        try:
            ...
            # 2. 从临时文件计算光衰减和波段积分 - 自适应深度分辨率
            self.log_message(f"  步骤2: 计算光衰减和波段积分 (自适应深度分辨率)")
            attenuation_file_path = self.calculate_light_attenuation_from_file_adaptive(matched_file_path, year)
            ...
            # 3. 从临时文件匹配N盐数据并计算N2O - 自适应深度处理
            self.log_message(f"  步骤3: 匹配N盐数据并计算N2O产率 (自适应深度处理)")
            n2o_file_path = self.match_and_calculate_n2o_from_file_adaptive(attenuation_file_path, year)
            ...
            self.log_message(f"  步骤4: 流式深度积分计算 (支持混合分辨率)")
            integration_file_path = self.calculate_depth_integration_streaming_adaptive(n2o_file_path, year)
            ...
            if n2o_data_info['total_rows'] > 0:
                ...
                self.log_message(f"    ✓ 使用自适应深度分辨率处理，未加载到内存")
    ...
    def calculate_light_attenuation_from_file_adaptive(self, matched_file_path: str, year: int) -> Optional[str]:
        """从临时文件读取匹配数据并计算光衰减，结果保存到临时文件 - 自适应深度分辨率版本"""
        self.log_message(f"    从临时文件计算 {year} 年光衰减和波段积分 (自适应深度分辨率)...")
        ...
        try:
            ...
            for chunk_idx, matched_chunk in enumerate(pd.read_csv(matched_file_path, chunksize=chunk_size)):
                ...
                for idx, row in matched_chunk.iterrows():
                    ...
                    # 使用自适应深度分辨率
                    depths, resolution_used, max_depth_used = self.calculate_depth_points(max_depth_available)

                    # 判断是否为浅水区域
                    is_shallow = max_depth_used < self.shallow_depth_threshold
                    if is_shallow:
                        shallow_locations += 1
                    else:
                    ...
                    for depth in depths:
                        depth_result = {
                            ...
                            'depth_resolution_used': resolution_used,
            ...
            if total_results > 0:
                ...
                self.log_message(
                    f"    深度分辨率统计: 浅水位置({shallow_locations}个), 标准位置({standard_locations}个)")
    ...
    def match_and_calculate_n2o_from_file_adaptive(self, attenuation_file_path: str, year: int) -> Optional[str]:
        """从临时文件读取光衰减数据，匹配N盐并计算N2O，结果保存到临时文件 - 自适应深度版本"""
        self.log_message(f"    从临时文件计算 {year} 年N2O产率 (自适应深度N盐匹配)...")
        ...
        try:
            ...
            shallow_water_count = 0
            shallow_water_matched_count = 0
            ...
            for chunk_idx, attenuation_chunk in enumerate(pd.read_csv(attenuation_file_path, chunksize=chunk_size)):
                ...
                for idx, row in attenuation_chunk.iterrows():
                    ...
                    lon = row['longitude']
                    depth = row['depth']
                    is_shallow = row.get('is_shallow_water', False)

                    if is_shallow:
                        shallow_water_count += 1

                    # 使用自适应N盐数据查找
                    n_salt_row = self.find_matching_n_salt_data_adaptive(
                        lat, lon, depth, n_salt_df, n_salt_spatial_index, is_shallow=is_shallow)
                    ...
                    if n_salt_row is not None:
                        ...
                        # 添加浅水处理标记
                        result_row['used_shallow_water_processing'] = is_shallow
                        if is_shallow:
                            result_row['n_salt_depth_used'] = 0.0  # 浅水区域使用0m深度的N盐数据
                        else:
                            result_row['n_salt_depth_used'] = n_salt_row['n_salt_depth']
                        ...
                        result_row['used_shallow_water_processing'] = is_shallow
                        result_row['n_salt_depth_used'] = np.nan
            ...
            if total_results > 0:
                ...
                shallow_match_rate = (shallow_water_matched_count / max(shallow_water_count, 1)) * 100
                ...
                self.log_message(
                    f"    浅水区域统计: {shallow_water_count:,} 个点, 匹配 {shallow_water_matched_count:,} 个 ({shallow_match_rate:.1f}%)")
                self.log_message(f"    ✓ 浅水区域已使用0m深度的N盐数据")
    ...
    def calculate_depth_integration_streaming_adaptive(self, n2o_file_path: str, year: int) -> Optional[str]:
        """真正的流式深度积分 - 支持混合分辨率的自适应版本"""
        self.log_message(f"    流式计算 {year} 年N2O深度积分 (支持混合分辨率)...")
        ...
        try:
                ...
                for idx, row in n2o_chunk.iterrows():
                    if pd.notna(row['N2O_total']) and row['N2O_total'] > 0:
                        ...
                        if len(location_buffer) >= max_locations_in_memory:
                            batch_results = self.process_location_buffer_adaptive(location_buffer, year)
            ...
            if processed_locations > 0:
                ...
                self.log_message(f"    ✓ 支持浅水区域1m分辨率和标准区域10m分辨率混合积分")
    ...
    def process_location_buffer_adaptive(self, location_buffer: Dict, year: int) -> List[Dict]:
        """处理位置缓冲区中的数据，计算积分 - 自适应版本"""
        batch_results = []
        ...
        for location_key, location_rows in location_buffer.items():
            ...
            # 计算单个位置的积分 - 自适应版本
            integration_result = self.calculate_single_location_integration_adaptive(location_rows, year)
            if integration_result:
                batch_results.append(integration_result)

        return batch_results

    def calculate_single_location_integration_adaptive(self, location_rows: List, year: int) -> Optional[Dict]:
        """计算单个位置的深度积分 - 自适应版本（支持混合分辨率）"""
        try:
            # 转换为DataFrame并按深度排序
            ...
            # 检查是否包含浅水数据（自适应处理）
            has_shallow_data = any(row.get('is_shallow_water', False) for row in location_rows)
            ...
            integration_result = {
                ...
                'has_shallow_water_data': has_shallow_data,
                'integration_depth_points': len(valid_n2o_data),
                'max_integration_depth': float(np.max(depths)),
                'min_integration_depth': float(np.min(depths))
            }
    ...
    def start_calculation(self):
        ...
        self.log_message("=== 开始N2O光产速率流式计算 (已修复所有问题 + 自适应深度分辨率 + N盐匹配信息) ===")
        ...
        self.log_message("✓ 新增特性: N盐匹配信息输出，包含匹配差异和浓度值")
        self.log_message("✓ 新增特性: 自适应深度分辨率 - 深度<10m使用1m分辨率，>=10m使用10m分辨率")
        self.log_message("✓ 新增特性: 浅水特殊处理 - 深度<10m使用0m的N盐数据作为整个水柱数据")
        ...
        try:
            ...
            self.log_message("=== 流式N2O计算全部完成! (所有问题已修复 + 自适应深度分辨率 + N盐匹配信息) ===")
            ...
            messagebox.showinfo("计算完成",
                                ...
                                f"✓ 新增特性:\n"
                                f"• N盐匹配信息输出：匹配差异、浓度值、深度处理标记\n"
                                f"• 自适应深度分辨率：深度<10m使用1m分辨率，>=10m使用10m分辨率\n"
                                f"• 浅水特殊处理：深度<10m使用0m的N盐数据作为整个水柱数据\n"
                                f"• 混合分辨率深度积分：支持1m和10m混合数据的梯形积分\n"
                                f"• 简化输出文件：只保留必要列，减少50%+存储空间\n\n"
                                f"深度处理说明:\n"
                                f"• 浅水阈值: {self.shallow_depth_threshold}m\n"
                                f"• 浅水分辨率: {self.shallow_depth_resolution}m\n"
                                f"• 标准分辨率: {self.standard_depth_resolution}m\n"
                                f"• 最大计算深度: {self.max_calculation_depth}m\n\n"
                                f"输出文件说明:\n"
                                f"• 浓度文件: {len(self.concentration_output_columns)} 列 (包含N盐匹配信息)\n"
                                f"• 积分文件: {len(self.integration_output_columns)} 列 (包含深度处理标记)\n"
                                f"• 存储空间节省: >50%\n\n"
                                f"内存优化特性:\n"
                                f"• 临时文件缓存在输出文件夹\n"
                                f"• 分块处理，减少99%内存占用\n"
