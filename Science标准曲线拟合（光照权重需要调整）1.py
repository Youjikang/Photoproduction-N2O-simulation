...
class FixedConversionN2OModel:
    ...
    def prepare_data(self, baseline_method='group_specific', focus_strategy='weighted'):
        ...
        data1 = {
            'UVB': [0, 0, 0, 0, 37082.46, 37082.46, 38441.09, 38441.09, 166397.9, 166397.9, 286023, 286023],
            'UVA': [0, 0, 0, 0, 1479018, 1479018, 1531166, 1531166, 6549466, 6549466, 11197932, 11197932],
            'VIS': [0, 0, 0, 0, 11133682, 11133682, 11361357, 11361357, 49642878, 49642878, 84125489, 84125489],
        ...
        data3 = {
            ...
            'UVB': [14900, 41200, 36700, 40900, 44002.26, 24158.09, 21257.85],
            'UVA': [679000, 1630000, 1460000, 1600000, 1707640, 1012472, 925981.6],
            'VIS': [4703816, 12264006, 11018119, 12017927, 12648742, 7350498, 6790609],
            'group': ['mixed'] * 7
        }
    ...
    def calculate_no3_conversion_factor(self):
        ...
        # 显示详细的拟合信息
        print(f"\n拟合方程:")
        print(f"NO2: N2O_rate = {slope_no2:.8f} × Light_intensity")
        print(f"NO3: N2O_rate = {slope_no3:.8f} × Light_intensity")
    ...
    def plot_conversion_analysis(self, results):
        ...
        fig.suptitle('固定NO3转换系数模型分析 (含指数函数模型)', fontsize=16)
    ...
    def predict_n2o(self, no2_conc, no3_conc, uvb, uva, vis):
        """预测N2O产生速率"""
        ...
        light_total = (uvb * 0.1 + uva * 0.3 + vis * 1.0) / 1e6
            ...
            if results and any(results.values()):
                ...
                if valid_results:
                    ...
                    for no2, no3, desc in test_cases:
                        pred = model.predict_n2o(no2, no3, 37000, 1500000, 11000000)
                        if pred is not None:
                ...
                if 'Fixed_Exponential' in valid_results:
                    ...
                    print(f"  c (光照系数): {params[2]:.4f}")
                    print(f"  d (光照偏移): {params[3]:.4f}")
                    print(f"  e (线性光照): {params[4]:.4f}")
                    print(f"  f (常数项): {params[5]:.4f}")
                    print(
                        f"  模型形式: N2O = {params[0]:.2f} * exp({params[1]:.3f} * equiv_NO2) * ({params[2]:.2f} * Light + {params[3]:.2f}) + {params[4]:.2f} * Light + {params[5]:.2f}")
